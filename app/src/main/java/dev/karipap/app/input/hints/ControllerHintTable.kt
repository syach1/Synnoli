package dev.karipap.app.input.hints
...
class ControllerHintTable internal constructor(
    ...
    fun lookup(vendorId: Int, productId: Int, buildModel: String): ControllerHint {
        ...
        if (exact != null) {
            dev.karipap.app.util.InputLog.write("[hints] vid+pid hit vid=$vendorId pid=$productId -> confirm=${exact.hint.menuConfirm} glyph=${exact.hint.glyphStyle}")
            return exact.hint
        }
        ...
        if (vidOnly != null) {
            dev.karipap.app.util.InputLog.write("[hints] vid hit vid=$vendorId -> confirm=${vidOnly.hint.menuConfirm} glyph=${vidOnly.hint.glyphStyle}")
            return vidOnly.hint
        }
        ...
        if (byModel != null) {
            dev.karipap.app.util.InputLog.write("[hints] model prefix '${byModel.modelPrefix}' hit Build.MODEL='$buildModel' -> confirm=${byModel.hint.menuConfirm} glyph=${byModel.hint.glyphStyle}")
            return byModel.hint
        }

        dev.karipap.app.util.InputLog.write("[hints] no match for vid=$vendorId pid=$productId Build.MODEL='$buildModel' -> default confirm=${default.menuConfirm} glyph=${default.glyphStyle}")
        return default
    }
