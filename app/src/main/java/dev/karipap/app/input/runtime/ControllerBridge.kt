...
class ControllerBridge(
    ...
    private val mappingRepository: dev.karipap.app.input.repo.MappingRepository? = null,
    private val blacklist: dev.karipap.app.input.ControllerBlacklist? = null,
    private val bundledCfgs: dev.karipap.app.input.autoconfig.BundledAutoconfigEntries? = null,
    ...
    private val pendingSavesById = mutableMapOf<Int, dev.karipap.app.input.DeviceMapping>()
    ...
    private val settleRunnable = Runnable {
        ...
        if (!initialEnumerationDone) {
            ...
            dev.karipap.app.util.InputLog.write("--- initial enumeration done ---")
        }
    }
    ...
    fun start(context: Context) {
        ...
        dev.karipap.app.util.InputLog.write("--- bridge start (Build.MODEL='$buildModel') ---")
    ...
    fun handleDeviceRemoved(androidDeviceId: Int) {
        dev.karipap.app.util.InputLog.write("event removed id=$androidDeviceId")
        scheduleSettle()
    }
    ...
    private fun settle(forcedFacts: List<DeviceFacts>? = null) {
        dev.karipap.app.util.InputLog.write("--- settle ---")
        ...
        for (id in existingEntryIds - targetEntryIds) {
            ...
            val displayName = snap?.mapping?.displayName?.takeIf { it.isNotEmpty() }
                ...
                ?: "Controller"
            val port = snap?.port
            dev.karipap.app.util.InputLog.write("  removed id=$id name='$displayName' port=${port?.let { "P${it + 1}" } ?: "-"}")
    ...
    ): dev.karipap.app.input.DeviceMapping {
    ...
    private fun cfgHintForName(
        ...
        skipVendorId: Int,
        skipProductId: Int,
    ): Pair<dev.karipap.app.input.hints.ControllerHint, Pair<Int, Int>>? {
