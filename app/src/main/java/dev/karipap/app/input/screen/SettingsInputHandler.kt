...
import dev.karipap.app.launcher.IntentAuditor
...
class SettingsInputHandler @Inject constructor(
    ...
    override fun onConfirm() {
            ...
            when {
                ...
                cat?.key == "kitchen" -> {
                    ...
                    val biosRootProvider = {
                        settings.biosDirectory.takeIf { it.isNotEmpty() }?.let { java.io.File(it) } ?: java.io.File(root, "BIOS")
                    }
                    if (!km.isRunning) km.toggle(root, context.assets, settings.kitchenCodeBypass, romsRootProvider, biosRootProvider)
        ...
        when (val key = settingsViewModel.enterSelected()) {
            "status_bar" -> settingsViewModel.enterSubCategory("status_bar", dev.karipap.app.R.string.settings_status_bar)
            "fgh_collection" -> settingsViewModel.enterSubCategory(
                "fgh_collection_picker",
                dev.karipap.app.R.string.setting_fgh_collection,
            ...
            "bios_directory" -> {
                val startPath = settings.biosDirectory.ifEmpty { settings.sdCardRoot }
                pushDirectoryBrowser(BrowsePurpose.BIOS_DIRECTORY, startPath)
            }
            ...
            "artwork_scraper" -> {
                settingsViewModel.save()
                launcherActions.openArtworkScraper()
            }
            "ra_username" -> {
    ...
    override fun onNorth() {
        ...
        } else if (item?.key == "bios_directory" && settings.biosDirectory.isNotEmpty()) {
            settingsViewModel.clearBiosDirectory()
            nav.dialogState.value = DialogState.RestartRequired
        }
    }
