...
class UpdateManager @Inject constructor(
    ...
    private fun loadCached(): UpdateInfo? {
        if (!updatesConfigured()) return null
    ...
    suspend fun checkForUpdate(): UpdateInfo? = withContext(Dispatchers.IO) {
        ...
        try {
            val json = fetchJson(BuildConfig.UPDATE_FEED_URL)
            val candidates = listOfNotNull("stable", "release").mapNotNull { key ->
    ...
    suspend fun downloadAndInstall(info: UpdateInfo) = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (downloadUrl.isEmpty()) {
                _downloadError.value = "Updates are not configured for this build."
    ...
    private fun updatesConfigured(): Boolean = BuildConfig.UPDATE_FEED_URL.isNotBlank()
