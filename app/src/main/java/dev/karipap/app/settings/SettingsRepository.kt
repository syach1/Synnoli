...
class SettingsRepository @Inject constructor(@ApplicationContext private val context: Context) {
    ...
    private fun loadFromDisk() {
        val file = dev.karipap.app.config.CannoliPaths(sdCardRoot).settingsJson
        settingsFile = file
        if (file.exists()) {
    ...
    var sdCardRoot: String
        get() = prefs.getString(KEY_SD_ROOT, null) ?: defaultRoot()
        set(value) {
            if (value == sdCardRoot) return
            prefs.edit().putString(KEY_SD_ROOT, value).apply()
            settingsFile = dev.karipap.app.config.CannoliPaths(value).settingsJson
            loadFromDisk()
        }

    private fun defaultRoot(): String {
        val karipapSettings = File(DEFAULT_ROOT, "Config/settings.json")
        val synnoliSettings = File(SYNNOLI_LEGACY_ROOT, "Config/settings.json")
        val legacySettings = File(LEGACY_ROOT, "Config/settings.json")
        return when {
            karipapSettings.exists() -> DEFAULT_ROOT
            synnoliSettings.exists() -> SYNNOLI_LEGACY_ROOT
            legacySettings.exists() -> LEGACY_ROOT
            else -> DEFAULT_ROOT
        }
    }
    ...
    var biosDirectory: String
        get() = jsonRead { optString(KEY_BIOS_DIRECTORY, "") }
        set(value) = jsonWrite { if (value.isEmpty()) remove(KEY_BIOS_DIRECTORY) else put(KEY_BIOS_DIRECTORY, value) }
    ...
    companion object {
        const val DEFAULT_ROOT = "/storage/emulated/0/Karipap/"
        private const val SYNNOLI_LEGACY_ROOT = "/storage/emulated/0/Synnoli/"
        private const val LEGACY_ROOT = "/storage/emulated/0/Cannoli/"
        ...
        private const val KEY_BIOS_DIRECTORY = "bios_directory"
