package dev.karipap.app.server
...
class FileServer(
    ...
    private val biosRootProvider: () -> File = { File(cannoliRoot, "BIOS") },
    ...
    private data class SupportedPlatform(val tag: String, val name: String)
    ...
    private fun handleClient(client: Socket) {
        try {
            ...
            when {
                ...
                method == "GET" && resource == "platforms" -> handlePlatforms(output)
                resource == "slots" -> {
                ...
                resource in RESOURCE_DIRS -> {
                    ...
                    val resourceRoot = when (resource) {
                        "roms" -> romsRootProvider()
                        "bios" -> biosRootProvider()
                        else -> File(cannoliRoot, baseDir)
                    }
                    val subpathSegments = normalizeResourceSubpath(resource, apiSegments.drop(1), resourceRoot)
                    val subpath = subpathSegments.joinToString("/")
    ...
    private fun handleInfo(output: OutputStream) {
        sendJson(output, 200, """{"name":"Karipap File Server","version":1}""")
    }
    ...
    private fun handleTags(output: OutputStream) {
        ...
        val tags = romsDir.listFiles { f -> f.isDirectory }
            ?.map { dir -> platforms.firstOrNull { PlatformFolderAliases.matches(it.tag, dir.name) }?.tag ?: dir.name }
    ...
    private fun handlePlatforms(output: OutputStream) {
        val romsDir = romsRootProvider()
        val platforms = supportedPlatforms().map { platform ->
            val exists = resolveExistingPlatformDir(romsDir, platform.tag)?.isDirectory == true
            """{"tag":"${escapeJson(platform.tag)}","name":"${escapeJson(platform.name)}","exists":$exists}"""
        }
        sendJson(output, 200, """{"platforms":[${platforms.joinToString(",")}]}""")
    }

    private fun normalizeResourceSubpath(resource: String, segments: List<String>, resourceRoot: File): List<String> {
        if (segments.isEmpty()) return segments
        val first = segments.first()
        return when {
            resource == "roms" && isSupportedPlatformTag(first) -> {
                val platformDir = resolveExistingPlatformDir(resourceRoot, first)
                if (platformDir?.exists() == true) listOf(platformDir.name) + segments.drop(1) else segments
            }
            resource == "bios" && isSupportedPlatformTag(first) -> {
                if (resolveExistingChild(resourceRoot, first)?.exists() == true) segments else segments.drop(1)
            }
            else -> segments
        }
    }

    private fun isSupportedPlatformTag(tag: String): Boolean =
        supportedPlatforms().any { it.tag.equals(tag, ignoreCase = true) }

    private fun supportedPlatforms(): List<SupportedPlatform> {
        return try {
            val json = org.json.JSONObject(assets.open("platforms.json").use { it.bufferedReader().readText() })
            val platforms = mutableListOf<SupportedPlatform>()
            for (tag in json.keys()) {
                val entry = json.getJSONObject(tag)
                val name = entry.optString("name", tag).ifEmpty { tag }
                platforms.add(SupportedPlatform(tag, name))
            }
            platforms.sortedWith(compareBy<SupportedPlatform> { it.name.lowercase(java.util.Locale.ROOT) }.thenBy { it.tag })
        } catch (_: Exception) {
            emptyList()
        }
    }

    private fun resolveExistingChild(parent: File, childName: String): File? {
        val direct = File(parent, childName)
        if (direct.exists()) return direct
        return parent.listFiles()?.firstOrNull { it.name.equals(childName, ignoreCase = true) }
    }

    private fun resolveExistingPlatformDir(parent: File, platformTag: String): File? {
        val direct = resolveExistingChild(parent, platformTag)
        if (direct?.exists() == true) return direct
        return parent.listFiles()?.firstOrNull { it.isDirectory && PlatformFolderAliases.matches(platformTag, it.name) }
    }
    ...
    private fun isSecure(file: File): Boolean {
        ...
        if (canonical.startsWith(romsCanonical)) return true
        val biosCanonical = try { biosRootProvider().canonicalPath } catch (_: Exception) { return false }
        return canonical.startsWith(biosCanonical)
    }
