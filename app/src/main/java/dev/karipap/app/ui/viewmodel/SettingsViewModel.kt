...
import dev.karipap.app.settings.BatteryDisplay
import dev.karipap.app.settings.ContentMode
import dev.karipap.app.settings.SettingsRepository
import dev.karipap.app.settings.TextSize
import dev.karipap.app.settings.TimeFormat
import dev.karipap.app.util.FontNameParser
...
class SettingsViewModel @Inject constructor(
    ...
    private fun buildFontOptions(): List<FontOption> = buildList {
        add(FontOption("default", "Default (JetBrainsMono Nerd Font)", appFonts.jetBrainsMono))
        add(FontOption("mplus_1_code", "M+ 1 Code", appFonts.mplus1Code))
    ...
    private fun resolveFont(): FontFamily {
        ...
        return fontOptions.firstOrNull { it.key == key }?.fontFamily ?: appFonts.jetBrainsMono
    }
    ...
    var updateInfo: dev.karipap.app.updater.UpdateInfo? = null
        set(value) {
            field = value
            reloadCategories()
        }
    ...
    fun cycleSelected(direction: Int, repeatCount: Int = 0) {
        ...
        when (item.key) {
            ...
            "kitchen_code_bypass" -> {
                ...
                dev.karipap.app.server.KitchenManager.setCodeBypass(settings.kitchenCodeBypass)
            }
            "ra_package" -> {
    ...
    fun clearBiosDirectory() {
        settings.biosDirectory = ""
        val catKey = _state.value.activeCategory ?: return
        _state.update { it.copy(items = buildItemsForCategory(catKey)) }
    }

    fun getColorEntries(): List<dev.karipap.app.ui.screens.ColorEntry> {
        val names = mapOf(
        ...
        return names.map { (key, labelRes) ->
            ...
            dev.karipap.app.ui.screens.ColorEntry(
                key = key,
                labelRes = labelRes,
                hex = hex,
    ...
    private fun captureSettings() = SettingsSnapshot(
        ...
        biosDirectory = settings.biosDirectory,
    ...
    private fun restoreSettings(snap: SettingsSnapshot) {
        ...
        settings.biosDirectory = snap.biosDirectory
    ...
    private fun buildItemsForCategory(categoryKey: String): List<SettingsItem> = when (categoryKey) {
        ...
        "library" -> buildList {
            ...
            add(SettingsItem("artwork_scraper", R.string.setting_artwork_scraper, isEditable = true, canCycle = false))
            ...
            add(SettingsItem("rom_directory", R.string.setting_rom_directory, valueText = romDir.ifEmpty { null }, valueRes = if (romDir.isEmpty()) R.string.value_cannoli_root else null, isEditable = true, canCycle = false, canClear = romDir.isNotEmpty()))
            val biosDir = settings.biosDirectory
            val effectiveBios = biosDir.ifEmpty { java.io.File(settings.sdCardRoot, "BIOS").absolutePath }
            add(SettingsItem("bios_directory", R.string.setting_bios_directory, valueText = effectiveBios, isEditable = true, canCycle = false, canClear = biosDir.isNotEmpty()))
        }
